...
Qt 前端停止收窄和展示 `content`，后端仍可返回该兼容字段。每条记录显示两行：第一行为操作名称和结果 Tag（标签），第二行为 `班组 / 作业人员：{班组} / {作业人员}`。直接复用已安装的 Ant Design `Timeline`，由组件内建节点和 rail（连接轨道）连接当前页相邻记录；页面只调整内容密度，删除 `border-bottom`，不手写圆点、伪元素或连接线几何样式。
...
- [连接线在分页边界中断] → 每页 `Timeline` 作为独立可见片段，组件只为当前页非末条目渲染内建 rail，最后一条自然不向分页控件延伸。
